package donald.fr.unitTest;

import donald.fr.modele.Quartier;
import donald.fr.modele.Joueur;
...
public class TestJoueur {
	...
	public void test2() {
		...
		Test.test(joueur.nbPieces() == 3,"test de l'ajout d'un nombre de pieces");
		joueur.ajouterPieces(-2);
		Test.test(joueur.nbPieces() == 3,"test d'ajout d'un nombre de pieces negatif");
		joueur.retirerPieces(-1);
		Test.test(joueur.nbPieces() == 3,"test d'un retrait d'un nombre negatif de pieces");
		joueur.retirerPieces(4);
		Test.test(joueur.nbPieces() == 3,"test de retrait d'un trop grand nombre de pieces");
		joueur.retirerPieces(2);
		joueur.retirerPieces(1);
		Test.test(joueur.nbPieces() == 0,"test de retrait d'un nombre de pieces");
	}
	...
	public void test5() {
		...
		Test.test(joueur.nbQuartiersDansMain()==0 && joueur.nbQuartiersDansCite()==0
				&& joueur.nbPieces()==0,"test de la reinitialisation");
	}
}