...
public class Jeu {
    ...
    private void gestionCouronne() {
        ...
        if(crown==-1) {
            ...
            Print.choice("\nchoisissez une option ");
            crown=Interaction.automatedChoice(nbJoueurs, false);
            plateauDeJeu.getJoueur(crown).setPossedeCouronne(true);
        } else {
            if(!nextKing.equals("")) {
                for (int i = 0; i < plateauDeJeu.getNombreJoueurs(); i++) {
                    if(plateauDeJeu.getJoueur(i).getNom().equals(nextKing)) crown=i;
                }
                nextKing="";
            } else {
